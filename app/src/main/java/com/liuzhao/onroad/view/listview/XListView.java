 ...
 */
package com.liuzhao.onroad.view.listview;
...
public class XListView extends ListView implements OnScrollListener {
	...
	private final static float OFFSET_RADIO = 3.0f; // support iOS like pull
													// feature.
	private final static float BOTTOM_OFFSET_RADIO = 6.0f;
	/** 显示刷新时间 */
	...
	public void setRefreshTime(boolean isNowTime) {
		...
		String dateStr = date.toLocaleString().toString();// 显示默认的刷新时间提示字符串
		if (!isNowTime) {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (getFirstVisiblePosition() == 0 && (mHeaderView.getVisiableHeight() > 0 || deltaY > 0)) {
				// the first item is showing, header has shown or pull down.下拉刷新
				updateHeaderHeight(deltaY / OFFSET_RADIO);
				invokeOnScrolling();
			} else if (getLastVisiblePosition() == mTotalItemCount - 1 && (mFooterView.getBottomMargin() > 0 || deltaY < 0)) {
				// last item, already pulled up or want to pull up.
				updateFooterHeight(-deltaY / BOTTOM_OFFSET_RADIO);
			}
			break;
		default:
			mLastY = -1; // reset
